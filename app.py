...
def load_data():
    ...
    # Nova base (PUBLICA_ABERTO_CAPES)
    # Fonte: https://docs.google.com/spreadsheets/d/1r-XXVn0wORZ1rsv4i_JJSSxsZyPytY8A
    file_path = "https://docs.google.com/spreadsheets/d/1r-XXVn0wORZ1rsv4i_JJSSxsZyPytY8A/export?format=xlsx"

    data = {}

    # Planilha nova: Wiley vem em duas abas (OnlineOpen + Gold) — consolidamos tudo em "Wiley"
    publishers = {
        '🟡 Elsevier': 'Elsevier',
        '🟢 Springer Nature': 'Springer Nature',
        '🟢 Wiley OnlineOpen': 'Wiley',
        '🟡 Wiley Gold': 'Wiley',
        '🟢 ACM': 'ACM',
        '🔵 IEEE': 'IEEE',
        '💎 ACS': 'ACS',
    ...
    for sheet_name, publisher in publishers.items():
        try:
            ...
            # A base nova já traz uma coluna "Editora" em várias abas.
            ...
            # Consolidar Wiley (duas abas) e também proteger contra sobrescrita acidental
            if publisher in data:
                data[publisher] = pd.concat([data[publisher], df], ignore_index=True)
            else:
                data[publisher] = df
...
def load_institutions_data():
    ...
    # IDs do Google Sheets (link atualizado)
spreadsheet_id = "1R1zxXygZqUxsVtW3pwJeZVltPRcTczKJ"

# Export CSV: sem gid = primeira aba (mais robusto). Mantemos fallback gid=0.
url_primary = f"https://docs.google.com/spreadsheets/d/{spreadsheet_id}/export?format=csv"
url_fallback = f"https://docs.google.com/spreadsheets/d/{spreadsheet_id}/export?format=csv&gid=0"
try:
        try:
