...
with st.expander("📋 Clique aqui para buscar seu periódico", expanded=True):
    ...
    # Informativo sobre busca na sidebar
    if search_term:
        st.info(f"🔍 **Filtro da sidebar ativo:** Buscando por '{search_term}'")
    ...
    if selected_publisher != "Todas":
        st.info(f"📚 **Filtro da sidebar ativo:** {selected_publisher}")
    ...
    editora_selecionada_page = st.selectbox(
        ...
        index=0 if selected_publisher == "Todas" else publishers_list.index(selected_publisher) + 1,
    ...
    # Usar busca da página ou sidebar
    busca_ativa_periodico = periodico_busca if periodico_busca else search_term
    editora_ativa = editora_selecionada_page if editora_selecionada_page != "Todas as Editoras" else selected_publisher
...
# Sincronizar variáveis da página com sidebar
search_term_final = periodico_busca if 'periodico_busca' in locals() else search_term
selected_publisher_final = editora_ativa if 'editora_ativa' in locals() else selected_publisher
