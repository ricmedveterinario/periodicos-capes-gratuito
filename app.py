...
def load_institutions_data():
    ...
    try:
        ...
        # Detectar nomes das colunas e padronizar
        columns = df.columns.tolist()
        
        # Mapear possíveis nomes de colunas
        col_map = {}
        
        # Coluna de nome da instituição
        if 'nome_completo' in columns:
            col_map['nome'] = 'nome_completo'
        elif 'nome' in columns:
            col_map['nome'] = 'nome'
        
        # Coluna de busca normalizada
        if 'nome_busca' in columns:
            col_map['nome_normalizado'] = 'nome_busca'
        elif 'nome_normalizado' in columns:
            col_map['nome_normalizado'] = 'nome_normalizado'
    ...
    if df_inst is not None:
        ...
        if not editoras_selecionadas:
            ...
            if instituicao_busca:
                ...
                # Buscar em nome_normalizado e acronimo
                mask = (
                    df_inst['nome_normalizado'].str.contains(termo_norm, na=False) |
                    df_inst['acronimo'].astype(str).str.upper().str.contains(termo_norm, na=False)
                )
            ...
            if len(resultados) > 0:
                ...
                for editora in editoras_encontradas:
                    ...
                    with st.expander(
                        ...
                        # Tabela - usar nome correto da coluna
...
try:
    ...
    try:
        ...
        pass  # Continuar mesmo se logo não existir
